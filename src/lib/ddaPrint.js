...
export function printDdaBoleto(boleto, invoiceOrInvoices) {
  const invoices = Array.isArray(invoiceOrInvoices) ? invoiceOrInvoices.filter(Boolean) : invoiceOrInvoices ? [invoiceOrInvoices] : [];
  const invoice = invoices[0];
  const title = `${boleto.document_number || "Boleto DDA"}`;
  const invoiceText = invoices.length > 0
    ? invoices.map((item) => `${item.document_type === "nfse" ? "NFS-e" : item.document_type === "cte" ? "CT-e" : "NF-e"} ${item.series ? `${item.series}/` : ""}${item.number || ""}`).join(" · ")
    : "Pendente de vínculo";
  ...
  const invoiceTotalValue = invoices.reduce((sum, item) => sum + (item.total_value || 0), 0);
  const invoiceRows = invoices.map((item) => `<div class="nf-item"><div><b>${safe(item.document_type === "nfse" ? "NFS-e" : item.document_type === "cte" ? "CT-e" : "NF-e")} ${safe(item.series ? `${item.series}/` : "")}${safe(item.number || "")}</b><small>${safe(item.supplier_name)} · Emissão ${safe(formatDate(item.issue_date))} · Venc. ${safe(formatDate(item.due_date))}</small></div><strong>${safe(formatCurrency(item.total_value))}</strong></div>`).join("");
  ...
  const html = `<!doctype html><html><head><title>${safe(title)}</title><style>
    ...
    <section class="boleto-card">
      ...
      <div class="nf">
        <div class="nf-head"><div><div class="label">Notas fiscais vinculadas</div><div class="nf-title">${safe(invoiceText)}</div></div><div class="nf-total">Total NFs: ${safe(formatCurrency(invoiceTotalValue || invoice?.total_value))}</div></div>
        ${invoices.length ? `<div class="nf-list">${invoiceRows}</div><div class="badges">${sigvBadge}${topconBadge}</div>` : `<div class="muted">Este boleto ainda não possui nota fiscal vinculada.</div>`}
      </div>
      <div class="bottom">
